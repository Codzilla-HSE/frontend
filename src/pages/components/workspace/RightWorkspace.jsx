...
export default function RightWorkspace({ position = 'right', submissions = [], matchOptions = null }) {
                ...
                <div className="panel-content">
                  {activeTab === 'description' ? (
                      <div className="task-description" style={{ padding: '16px', color: '#f8fafc' }}>
                        {matchOptions ? (
                            <>
                              <h3>{matchOptions.title || "Без названия"}</h3>
                              <div style={{ whiteSpace: 'pre-wrap' }}>{matchOptions.statement}</div>
                            </>
                        ) : (
